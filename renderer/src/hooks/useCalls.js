...
export default function useCalls() {
    ...
    const handleIncoming = useCallback((data) => {
        setCall((prev) => {
            // A richer telephony packet may follow the generic HFP ringing packet — keep existing
            // caller info when the incoming packet carries no details.
            if (prev && prev.number && prev.number !== 'Unknown Number') return prev;
            return {
                id: `call_${Date.now()}`,
                type: 'incoming',
                status: 'RINGING',
    ...
    const handleTalking = useCallback((data) => {
        setCall((prev) => {
            if (prev) {
                ...
                const type = prev.status === 'RINGING' ? 'incoming' : prev.type || 'outgoing';
                return {
                    ...prev,
                    type,
                    status: 'ACTIVE',
            ...
            return {
                ...
                thumbnail: data.phoneThumbnail || null,
                startedAt: Date.now(),
                isMuted: false,
                audioTarget: 'PC_SPEAKERS'
            };
